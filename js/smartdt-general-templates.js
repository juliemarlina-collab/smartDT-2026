(function(){
  ...
  function field(f){
    if(f[0]==='grid' || f[0]==='grid4'){
      ...
      return '<div class="'+cls+'">'+f[2].split('|').map((x,i)=>'<div class="field"><label>'+esc(x)+'</label><textarea class="textarea small" name="'+esc(f[3])+'_'+(i+1)+'" placeholder="Write here..."></textarea></div>').join('')+'</div>';
    }
  ...
  function sampleFor(code){
    const samples = {
      ...
      T02:'Example: Ask “Can you tell me about the last time you faced this problem?” and follow up with “Why was that difficult?”',
      T03:'Example quote: “I wanted to join, but I was not sure where to start.” Highlight this as evidence of uncertainty.',
      T04:'Example: Say = “I am confused”; Do = asks friends; Think = worries about mistakes; Feel = nervous but interested.',
      T05:'Example persona: A first-time user who is curious but needs clear steps, reassurance and simple guidance.',
      T06:'Example statement: A first-time user needs a simple step-by-step guide because they feel unsure and do not want to make mistakes.',
      T07:'Example branches: information, support, digital guide, signage, peer help, quick demo, checklist.',
      T08:'Example sketch: A simple three-step mobile screen showing Start, Guide Me and Submit Evidence.',
      T09:'Example SCAMPER: Combine checklist + video demo; eliminate confusing wording; reverse the flow so users choose goals first.',
      T10:'Example: Choose the idea with high user impact, realistic build effort and clear value.',
      T11:'Example prototype: A clickable mock-up or paper model that shows the user journey and key feature.',
      T12:'Example feedback: What worked — easy steps. Improve — button label unclear. Action — rename button and add hint.',
      T13:'Example pitch: Problem → evidence → insight → solution → prototype → feedback → impact → call to action.'
    };
  ...
  document.addEventListener('click', function(e){
    const btn = e.target.closest('.template-mode-switch button');
    if(!btn) return;
    const card = btn.closest('.smart-template-card');
    if(!card) return;
    card.querySelectorAll('.template-mode-switch button').forEach(b=>b.classList.toggle('active', b===btn));
    const mode = btn.dataset.mode;
    card.querySelectorAll('.mode-panel').forEach(p=>{ p.hidden = p.dataset.panel !== mode; });
  });
})();
